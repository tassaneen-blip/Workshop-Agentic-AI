(() => {
  const history = [], messages = document.querySelector('#messages'), empty = document.querySelector('#empty');
  const add = (role, content) => { if (empty) empty.remove(); const el = document.createElement('div'); el.className = `bubble ${role}`; el.textContent = content; messages.appendChild(el); messages.scrollTop = messages.scrollHeight; };
  document.querySelector('#form').addEventListener('submit', async (event) => {
    ...
    const provider = document.querySelector('input[name="provider"]:checked').value; const model = document.querySelector('#model').value.trim(); input.value = ''; add('user', message);
